...
// Funkcija za konvertovanje datuma u UNIX timestamp
const toUnixTimestamp = (dateString) => {
  const date = new Date(dateString + " 2025"); // Dodaj godinu
  return Math.floor(date.getTime() / 1000); // Pretvori u sekunde
};

const OCCASION_DATE = 1764370800; // UNIX timestamp za "29. Novembar 2025."
...
describe("TokenMaster", () => {
  ...
  describe("Occasions", () => {
    ...
    it("Returns occasions count", async () => {
      ...
      expect(occasion.tickets.toNumber()).to.be.equal(OCCASION_MAX_TICKETS);
  ...
  describe("Refunding", () => {
    ...
    it("Refunds the last purchase", async () => {
      ...
      const gasUsed = refundReceipt.cumulativeGasUsed.mul(
        refundReceipt.effectiveGasPrice
      );
